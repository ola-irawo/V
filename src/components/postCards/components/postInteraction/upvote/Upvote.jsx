...
const Upvote = ({posts}) => {
    const {_id, totalUpvotes } = posts
    const currentUser = getCurrentUser()
    const dispatch = useDispatch()
    const buttonRef = useRef()
    const upvotedUserIds = new Set(totalUpvotes);
    const hasUpvotedPost = upvotedUserIds.has(currentUser.userUid);
  ...
  return (
    <Button 
        ...
        <div aria-label="Upvote details">
            <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 25 24" fill={hasUpvotedPost ? "#000" : "none"}>
            ...
            {totalUpvotes?.length > 0 && <span aria-label={`Total number of upvotes: ${getTotalReactionsCount(totalUpvotes)}` }>{getTotalReactionsCount(totalUpvotes)}</span>}
        </div>
    }
    />
  )
